    ...
    def request(self, options: HttpRequestOptions) -> object:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                wrapped = TimeoutError(f"Request timed out for {url}", cause=error)
                ...
                wrapped = NetworkError(f"Network request failed for {url}", cause=error)
                self._run_error_hooks(context, wrapped)
                if self._should_retry(
                    retry_policy,
                    RetryDecisionContext(
...
def _resolve_retry_policy(
    ...
    return RetryPolicy(
        ...
        retry_methods=override.retry_methods or default_retry.retry_methods,
        retry_on_statuses=override.retry_on_statuses or default_retry.retry_on_statuses,
        ...
        should_retry=override.should_retry or default_retry.should_retry,
    )
...
def _should_retry(
    ...
    if context.status is not None:
        if context.status not in retry_policy.retry_on_statuses:
            return False
    elif context.error is not None and not retry_policy.retry_on_network_error:
        return False
